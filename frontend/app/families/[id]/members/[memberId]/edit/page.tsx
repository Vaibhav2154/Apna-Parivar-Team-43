...
import { updateFamilyMember, getFamilyMember } from '@/lib/family-service';
...
export default function EditMemberPage() {
  ...
  const loadMember = async () => {
    try {
      setIsLoading(true);
      const member = await getFamilyMember(familyId, memberId);
      setFormData({
        name: member.name,
      ...
      setRelationshipFields(relFields.length > 0 ? relFields : [{ key: '', value: '' }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateFamilyMember(familyId, memberId, {
        name: formData.name,
    ...
    <ProtectedRoute requiredRole={['family_admin', 'family_co_admin']}>
      <div className="min-h-screen bg-background py-12 sm:py-16 lg:py-20">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <Link href={`/families/${familyId}/members/${memberId}`} className="text-white/60 hover:text-white mb-4 inline-flex items-center gap-2 transition">
              ← Back to Member
            </Link>
            <h1 className="text-4xl sm:text-5xl font-bold text-foreground">Edit Member</h1>
          </div>
